...
2. The user may execute **Preview Bundle** to render the current generated markdown in the
   workarea.
3. The user executes **Export Bundle**, which prompts for a save location for the generated
   markdown (`.md`) file.
4. BundlerMD reads the current contents of the added files and combines them into a single
...
- **Add detected language tag to code fences** — checkbox, on by default. When checked,
  opening code fences append a GitHub Linguist-style language tag when BundlerMD can
  infer one from the filename, extension, or content.
...
- **Missing files**: entries whose underlying file no longer exists are highlighted in red.
  ...
  handling).
- **Preview Bundle**: an icon button in the toolbar generates the current bundle text and
  replaces the workarea with a rendered markdown preview. The preview has an `x` control at
  the top right that closes it and returns to the file list.
...
Preview generation uses the same in-memory bundle text as Export and Copy to Clipboard. The
frontend renders that markdown to HTML with raw HTML disabled, and applies syntax highlighting
to fenced code blocks when the fence's language tag is supported by the highlighter. Code
fence tags generated by the **Add detected language tag to code fences** setting are therefore
used directly by the preview when possible.

Preview does not mutate project state or write files. If some files cannot be included, the
same problems dialog flow is used, with **Preview anyway** rendering the bundle minus the
listed files.
...
{{ fence }}{{ optional_linguist_language_tag }}
...
- `fence`: scan the file's content for its longest uninterrupted run of backticks; the fence
  uses that count plus one, with a minimum of 3. When **Add detected language tag to code
  fences** is checked and a language can be inferred from the filename, extension, or content,
  the opening fence appends Linguist's default language alias as its Markdown info string.
  Headings or other markdown inside a file's content live within its code fence, which is
  acceptable.
...
Copy and Preview use the same generated bundle and problem detection. Their continue buttons
are **Copy anyway** and **Preview anyway**, respectively.
